...
func _build_bound_rule_stack(progress_ratio: float = 0.30) -> Array[Variant]:
	...
	var recovery_sequence_generator := RecoverySequenceGenerator.new()
...
func _add_hazard(spawner: HazardSpawner, hazard_type: StringName, position: Vector2) -> Node2D:
...
func test_resolve_frame_when_rock_hits_then_damage_and_failure_handoff_match_scene_behavior() -> void:
	...
	var run_state := bound_rule_stack[0] as RunState
	var run_director := bound_rule_stack[1] as RunDirector
...
func test_resolve_frame_when_head_on_hazard_passes_close_then_dodge_and_near_miss_award_once() -> void:
	...
	assert_eq(run_state.bonus_score, RunState.NEAR_MISS_BONUS_SCORE)
