...
const TEST_BEST_RUN_SAVE_PATH := SavePaths.TEST_RUN_STATE_BEST_RUN_SAVE_PATH
# Public Methods
...
## Verifies defaults match expected mvp boot values.
...
## Verifies reset for new run restores all core run values.
...
## Verifies delivery progress ratio tracks route completion.
...
## Verifies configure route distance updates starting distance.
...
## Verifies start failure creates a single active failure record.
...
## Verifies cannot start second failure until current failure is cleared.
...
## Verifies tick failure increments elapsed time for active failure.
...
## Verifies recovery sequence tracks prompt progress and completion.
...
## Verifies perfect recovery bonus when awarded then bonus score is added.
...
## Verifies recovery sequence timeout counts down and triggers expiry.
...
## Verifies current recovery perfect when wrong input or timeout then bonus eligibility is lost.
...
## Verifies recovery failure penalty applies resource losses and instability.
...
## Verifies recovery success clears failure without penalty.
...
## Verifies recover speed restores forward speed over time.
...
## Verifies recovery transients clear outcome and cooldown over time.
