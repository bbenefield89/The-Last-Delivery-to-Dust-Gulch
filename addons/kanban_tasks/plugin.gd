...
## Saves the editor board to its configured file.
func __editor_save_board() -> void:
	var ctx: __EditContext = __Singletons.instance_of(__EditContext, self)
	__save_board(ctx.settings.editor_data_file_path)


## Reloads the editor board from disk without saving first.
func __editor_reload_board() -> void:
	var ctx: __EditContext = __Singletons.instance_of(__EditContext, self)
	var editor_data_file_path = ctx.settings.editor_data_file_path
	if editor_data_file_path.is_empty() or not FileAccess.file_exists(editor_data_file_path):
		return
	if not board_changed:
		__reload_editor_board()
	else:
		__request_discard_changes(__reload_editor_board)


## Creates a fresh editor board and persists it to the configured file.
...
## Reopens the configured editor board file from disk.
func __reload_editor_board() -> void:
	var ctx: __EditContext = __Singletons.instance_of(__EditContext, self)
